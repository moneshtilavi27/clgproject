...
class Admin_Operation extends CI_Model
{
    ...
    public function getDataId($table, $attr, $id)
    {
     ...
     {
        if($dept=="Select" && $modeOfExam=="Select" && $year=="Select" && $role==0)
        {
            $this->db->select('f.fid,f.faculty_id,f.fimage,f.name,f.department,w.wid,w.year,w.modeOfExam,r.rolename,w.scriptDate,w.evaluvated_script,w.scheme,w.question_paper,w.add_comment,w.status,p.rate,s.subject,s.sub_code');
            ...
            $this->db->where('w.modeOfExam',$modeOfExam);
            $this->db->where('w.year',$year);
            $this->db->where('r.roleid',$role);
            $this->db->where('w.status',0);
            $query = $this->db->get();
        }
        return $query;
    ...
    public function getRemuData($fid,$department,$year,$roleid)
    {
        if($fid!="" and $department!="" and $year!="" and $roleid!=""){
            $this->db->select('f.fid,f.faculty_id,f.fimage,f.name,f.department,f.bankacc,w.year,w.modeOfExam,r.rolename,w.scriptDate,w.evaluvated_script,w.scheme,w.question_paper,w.add_comment,w.status,p.rate,s.subject,s.sub_code');
            $this->db->from('faculty f');
            $this->db->join('facultywork w', 'f.fid = w.fid');
            $this->db->join('subject s', 's.sub_id = w.sub_id');
            $this->db->join('pricerate p', 'w.roleid = p.roleid');
...
public function profile($id)
{
    ...
    public function getFacultyAjax($depart)
    {  
        $this->db->distinct();   
        $this->db->select("f.name,f.faculty_id,f.fid");
        $this->db->from('faculty f');
        $this->db->join('facultywork w', 'w.fid = f.fid');
        $this->db->where('f.department',$depart); 
        $query = $this->db->get();  
        if ($query->num_rows() > 0) {
            foreach ($query->result() as $row) {
                $data[] = $row;
